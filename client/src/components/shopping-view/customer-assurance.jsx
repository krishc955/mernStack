...
function CustomerAssurance() {
  ...
  return (
    <section className="py-12 lg:py-16 bg-gradient-to-br from-amber-25 via-cream-50 to-amber-50" style={{ backgroundColor: '#fefcf8' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header - Mobile Optimized */}
        <div className="text-center mb-8 lg:mb-12">
          <h2 className="text-2xl lg:text-3xl xl:text-4xl font-extrabold text-amber-900 mb-3 lg:mb-4 tracking-tight">
            Why Choose StyleHub?
          </h2>
          <p className="text-amber-700/80 text-sm lg:text-lg font-medium max-w-2xl mx-auto leading-relaxed tracking-wide px-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8">
          {assuranceItems.map((item, index) => (
            <div 
              key={index}
              className={`relative bg-white/70 backdrop-blur-sm rounded-xl lg:rounded-2xl p-4 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 lg:hover:-translate-y-2 border border-amber-100/50 group ${item.hoverBg}`}
            >
              ...
              <div className="text-center">
                <h3 className="text-sm lg:text-xl font-bold text-amber-900 mb-2 lg:mb-3 tracking-tight leading-tight">
                  {item.title}
                </h3>
                <p className="text-amber-700/85 text-xs lg:text-sm leading-relaxed font-medium tracking-wide hidden sm:block">
                  {item.description}
                </p>
                {/* Simplified mobile description */}
                <p className="text-amber-700/85 text-xs leading-relaxed font-medium tracking-wide sm:hidden">
                  ...
                  {item.title === "Fast Shipping" && "Quick delivery tracking"}
                </p>
              </div>
              ...
              <div className="absolute top-2 right-2 lg:top-4 lg:right-4 opacity-10 group-hover:opacity-20 transition-opacity duration-300 hidden sm:block">
                <CheckCircle className="h-4 w-4 lg:h-6 lg:w-6 text-amber-600" />
              </div>
            </div>
          ))}
        </div>
