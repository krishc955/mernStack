...
function ShoppingOrders() {
  ...
  function handleDeleteAllPendingOrders() {
    const pendingCount = orderList?.filter(order => order.orderStatus === "pending").length || 0;
    if (pendingCount === 0) {
      toast({
        title: "No Pending Orders",
        description: "There are no pending orders to delete.",
        variant: "destructive",
      });
      return;
    }

    if (window.confirm(`Are you sure you want to delete all ${pendingCount} pending orders? This action cannot be undone.`)) {
      dispatch(deleteAllPendingOrders(user?.id)).then((data) => {
        if (data?.payload?.success) {
          toast({
            title: "All Pending Orders Deleted",
            description: `${data?.payload?.deletedCount || pendingCount} pending orders have been removed successfully.`,
          });
          // If showing pending only and all are deleted, switch to show all
          if (showPendingOnly) {
          ...
          toast({
            title: "Error",
            description: data?.payload?.message || "Failed to delete pending orders",
            variant: "destructive",
          });
        }
      });
          ...
          <div className="flex items-center gap-2">
            ...
            {pendingOrdersCount > 0 && (
              ...
              >
                ...
                Delete All Pending ({pendingOrdersCount})
              </Button>
            )}
          </div>
        </div>
